...
class Lazada():
    def __init__(self):
        self.__api_key = b'1ODrnT8RXreK2Pd-0S8NvLZnOkYcmKnhFJM_YOzPf4uWlm27dZyMmwtO'
        self.__user_id = 'billaeon@qq.com'
    ...
    def complete_sku(self, cb_sku, cid):
        self.__mongodb.pre_citimall_upload.replace_one({'sku': cb_sku},
                                              {'sku': cb_sku,
                                               'status': 1,
                                               "cid": cid}, True)
...
if __name__ == "__main__":
    ...
    while True:
        row = py.pre_citimall_upload.find_one({'status': 0})
        if not row:
            break
        try:
